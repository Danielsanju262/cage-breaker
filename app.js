...
const LEVEL_CONFIG = [
    { level: 1,  title: "Newly Engaged",              minXP: 0 },
    { level: 2,  title: "Vision Boarder",             minXP: 130 },
    { level: 3,  title: "Date Picker",                minXP: 390 },
    { level: 4,  title: "Budget Scout",               minXP: 800 },
    { level: 5,  title: "Guest List Compiler",        minXP: 1370 },
    { level: 6,  title: "Venue Explorer",             minXP: 2120 },
    { level: 7,  title: "Theme Designer",             minXP: 3060 },
    { level: 8,  title: "Vendor Hunter",              minXP: 4200 },
    { level: 9,  title: "Schedule Architect",         minXP: 5560 },
    { level: 10, title: "Event Coordinator",          minXP: 7140 },
    { level: 11, title: "Logistics Strategist",       minXP: 8960 },
    { level: 12, title: "Ceremony Planner",           minXP: 11040 },
    { level: 13, title: "Reception Master",           minXP: 13380 },
    { level: 14, title: "Playlist Maestro",           minXP: 16000 },
    { level: 15, title: "Seating Tactician",          minXP: 18900 },
    { level: 16, title: "Rehearsal Director",         minXP: 22100 },
    { level: 17, title: "Timeline Specialist",        minXP: 25610 },
    { level: 18, title: "VIP Host",                   minXP: 29450 },
    { level: 19, title: "Details Perfectionist",      minXP: 33620 },
    { level: 20, title: "Grand Wedding Planner",      minXP: 38130 },
    { level: 21, title: "Diamond Concierge",          minXP: 43000 },
    { level: 22, title: "Royal Orchestrator",         minXP: 48230 },
    { level: 23, title: "Master of Ceremonies",       minXP: 53840 },
    { level: 24, title: "Ultimate Visionary",         minXP: 59840 },
    { level: 25, title: "Legendary Wedding Sovereign",minXP: 66230 }
];
...
const app = {
    ...
    toggleTaskExpand(taskId, eventObj) {
        ...
        this.checkAndShowGreatestCrackModal(() => {
            this._doToggleTaskExpand(taskId);
        });
    },

    _doToggleTaskExpand(taskId) {
    ...
    // ==== GREATEST CRACK OF THE DAY LOGIC ====
    getTodayDateString() {
        const d = new Date();
    ...
    getDailyCrack() {
        ...
        let crack = (state && state.dailyCrack) ? state.dailyCrack : null;
        if (!crack) {
            try {
                const saved = localStorage.getItem('daily_greatest_crack');
                if (saved) crack = JSON.parse(saved);
            } catch (e) {}
        }
        
        // Date check: if new day, reset crack data for today
        if (!crack || crack.date !== today) {
            crack = {
                date: today,
                what: '',
                how: '',
    ...
    saveDailyCrack(crack) {
        if (state) {
            state.dailyCrack = crack;
            if (!state.dailyCrackHistory) state.dailyCrackHistory = [];
            const idx = state.dailyCrackHistory.findIndex(h => h.date === crack.date);
            const historyEntry = { ...crack, updatedAt: new Date().toISOString() };
            if (idx >= 0) {
                state.dailyCrackHistory[idx] = { ...state.dailyCrackHistory[idx], ...historyEntry };
            } else {
                state.dailyCrackHistory.push(historyEntry);
            }
            syncState(state);
        }
        try {
            localStorage.setItem('daily_greatest_crack', JSON.stringify(crack));
            if (state && state.dailyCrackHistory) {
                localStorage.setItem('daily_crack_history', JSON.stringify(state.dailyCrackHistory));
            }
        } catch (e) {}
    },

    getDailyCrackHistory() {
        let history = (state && Array.isArray(state.dailyCrackHistory)) ? state.dailyCrackHistory : [];
        if (!history.length) {
            try {
                const saved = localStorage.getItem('daily_crack_history');
                if (saved) history = JSON.parse(saved);
            } catch (e) {}
        }
        
        const today = this.getTodayDateString();
        const todayCrack = this.getDailyCrack();
        if (todayCrack && todayCrack.what && !history.some(h => h.date === today)) {
            history.push({ ...todayCrack, updatedAt: new Date().toISOString() });
        }
        
        return history.slice().sort((a, b) => b.date.localeCompare(a.date));
    },

    saveDailyJournalNote(dateStr, noteText) {
        if (state) {
            if (!state.dailyCrackHistory) state.dailyCrackHistory = [];
            const item = state.dailyCrackHistory.find(h => h.date === dateStr);
            if (item) {
                item.notes = noteText;
                item.updatedAt = new Date().toISOString();
            } else {
                state.dailyCrackHistory.push({
                    date: dateStr,
                    what: '',
                    how: '',
                    achieved: false,
        ...
        this.renderDailyJournalHistoryHTML();
    },

    getWeeklyJournalEntries() {
        let entries = (state && Array.isArray(state.weeklyJournal)) ? state.weeklyJournal : [];
        if (!entries.length) {
            try {
                const saved = localStorage.getItem('weekly_journal_entries');
    ...
    getCurrentWeekRangeLabel() {
        ...
        return `Week of ${format(monday)} - ${format(sunday)}, ${sunday.getFullYear()}`;
    },

    saveWeeklyReflection(editId = null) {
        const weekInput = document.getElementById('weekly-label-input');
        const workedInput = document.getElementById('weekly-worked-input');
        const didntInput = document.getElementById('weekly-didnt-input');
        
        const weekLabel = weekInput ? weekInput.value.trim() : '';
        const whatWorked = workedInput ? workedInput.value.trim() : '';
        const whatDidntWork = didntInput ? didntInput.value.trim() : '';

        if (!whatWorked && !whatDidntWork) {
            if (workedInput) {
                workedInput.style.borderColor = '#e74c3c';
                workedInput.focus();
            }
            return;
        }
        if (workedInput) workedInput.style.borderColor = '';

        if (state) {
            if (!state.weeklyJournal) state.weeklyJournal = [];
            if (editId) {
                const entry = state.weeklyJournal.find(w => w.id === editId);
                if (entry) {
                    entry.weekLabel = weekLabel || entry.weekLabel;
                    entry.whatWorked = whatWorked;
                    entry.whatDidntWork = whatDidntWork;
                    entry.updatedAt = new Date().toISOString();
                }
            } else {
                state.weeklyJournal.push({
                    id: 'wk_' + Date.now(),
                    weekLabel: weekLabel || this.getCurrentWeekRangeLabel(),
        ...
        try {
            if (state && state.weeklyJournal) {
                localStorage.setItem('weekly_journal_entries', JSON.stringify(state.weeklyJournal));
            }
        } catch (e) {}

        this.renderWeeklyJournalHistoryHTML();
    },

    deleteWeeklyReflection(id) {
        if (!confirm('Are you sure you want to delete this weekly reflection?')) return;
        if (state && Array.isArray(state.weeklyJournal)) {
            state.weeklyJournal = state.weeklyJournal.filter(w => w.id !== id);
            syncState(state);
        }
        try {
            if (state && state.weeklyJournal) {
    ...
    activeJournalTab: 'today',

    switchJournalTab(tabName) {
        this.activeJournalTab = tabName;
        const tabToday = document.getElementById('tab-today-crack');
        const tabDaily = document.getElementById('tab-daily-journal');
        const tabWeekly = document.getElementById('tab-weekly-journal');
        
        const viewToday = document.getElementById('journal-view-today');
        const viewDaily = document.getElementById('journal-view-daily');
        const viewWeekly = document.getElementById('journal-view-weekly');

        if (tabToday) tabToday.classList.toggle('active', tabName === 'today');
        if (tabDaily) tabDaily.classList.toggle('active', tabName === 'daily');
        if (tabWeekly) tabWeekly.classList.toggle('active', tabName === 'weekly');

        if (viewToday) viewToday.style.display = (tabName === 'today') ? 'flex' : 'none';
        if (viewDaily) viewDaily.style.display = (tabName === 'daily') ? 'flex' : 'none';
        if (viewWeekly) viewWeekly.style.display = (tabName === 'weekly') ? 'flex' : 'none';
    ...
    openJournalModal(tab = 'today') {
        const crack = this.getDailyCrack();
        const modalTitle = document.getElementById('crack-modal-title');
        const modalSub = document.getElementById('crack-modal-subtitle');
        const inputView = document.getElementById('crack-input-view');
        const reminderView = document.getElementById('crack-reminder-view');
        const displayWhat = document.getElementById('crack-display-what');
        const displayHow = document.getElementById('crack-display-how');

        if (!crack.what || !crack.what.trim()) {
            if (modalTitle) modalTitle.textContent = "What will your greatest crack be today?";
            if (modalSub) modalSub.textContent = "It can be something that you haven't tried before or something new that you have not done and can cause a biggest crack";
            if (inputView) inputView.style.display = 'flex';
            if (reminderView) reminderView.style.display = 'none';
        } else {
            if (modalTitle) modalTitle.textContent = "Today's Greatest Crack Focus";
            if (modalSub) modalSub.textContent = "Here is the biggest breakthrough you committed to delivering today. Keep pushing!";
            if (inputView) inputView.style.display = 'none';
            if (reminderView) reminderView.style.display = 'flex';
            if (displayWhat) displayWhat.textContent = crack.what;
            if (displayHow) displayHow.textContent = crack.how || 'No plan details provided.';
        }

        this.openModal('greatest-crack-modal');
        this.switchJournalTab(tab);
    },

    renderDailyJournalHistoryHTML() {
        const container = document.getElementById('journal-view-daily');
        if (!container) return;
        
        const history = this.getDailyCrackHistory();
        if (!history || history.length === 0) {
            container.innerHTML = `
                <div style="text-align: center; padding: 3rem 1rem; color: var(--text-muted);">
                    <span class="material-icons-round" style="font-size: 2.5rem; color: rgba(243, 156, 18, 0.4); margin-bottom: 0.5rem;">event_note</span>
                    <p style="font-size: 0.95rem; font-weight: 600; color: #fff; margin: 0.2rem 0;">No Daily Crack Entries Yet</p>
                    <p style="font-size: 0.82rem; margin: 0;">As you commit to daily cracks and log breakthroughs, your daily history will appear here!</p>
                </div>
            `;
            return;
        }
        ...
        const html = history.map(item => {
            ...
            const formattedDate = !isNaN(dObj.getTime()) ? dObj.toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric', year: 'numeric' }) : item.date;
            const isToday = item.date === this.getTodayDateString();

            return `
                <div class="daily-journal-card">
                    <div class="daily-journal-header">
                        <div class="daily-date-tag">
                            <span class="material-icons-round" style="font-size: 1rem; color: #f39c12;">calendar_today</span>
                            <span>${formattedDate} ${isToday ? '<span style="font-size: 0.7rem; background: rgba(243,156,18,0.2); color: #f39c12; padding: 0.1rem 0.4rem; border-radius: 4px; font-weight: 800; margin-left: 0.3rem;">TODAY</span>' : ''}</span>
                        </div>
                        <span class="daily-status-badge ${item.achieved ? 'achieved' : 'pending'}">
                            ${item.achieved ? '✓ Achieved' : '• In Progress'}
                        </span>
                    </div>
                    ${item.what ? `
                    <div class="crack-card-section">
                        <span class="crack-section-tag">WHAT IT CAN BE</span>
                        <p class="crack-text-content" style="font-size: 0.88rem;">${item.what}</p>
                    </div>
                    ` : ''}
                    ${item.how ? `
                    <div class="crack-card-section">
                        <span class="crack-section-tag">HOW PLANNING TO DO IT</span>
                        <p class="crack-text-content" style="font-size: 0.88rem; opacity: 0.9;">${item.how}</p>
                    </div>
                    ` : ''}
                    <div class="daily-journal-note-box">
                        <label style="font-size: 0.72rem; font-weight: 700; color: var(--text-muted); display: block; margin-bottom: 0.25rem;">DAILY REFLECTION NOTE:</label>
                        <textarea class="custom-input auto-resize" placeholder="Add daily notes or evidence reflections..." rows="2" style="font-size: 0.83rem; padding: 0.35rem 0.5rem;" onchange="app.saveDailyJournalNote('${item.date}', this.value)">${item.notes || ''}</textarea>
                    </div>
                </div>
            `;
        }).join('');
        ...
        container.innerHTML = `
            <div style="display: flex; align-items: center; justify-content: space-between;">
                <span style="font-size: 0.8rem; font-weight: 700; color: var(--text-muted); text-transform: uppercase; letter-spacing: 0.8px;">Past Daily Cracks (${history.length})</span>
                <button type="button" class="btn-ghost btn-small" onclick="app.switchJournalTab('today')">+ New Daily Crack</button>
            </div>
            ${html}
        `;
    },

    renderWeeklyJournalHistoryHTML() {
        const container = document.getElementById('journal-view-weekly');
        if (!container) return;

        const entries = this.getWeeklyJournalEntries();
        const defaultWeekLabel = this.getCurrentWeekRangeLabel();

        const formHTML = `
            <div class="weekly-reflection-form">
                <div style="display: flex; align-items: center; justify-content: space-between; border-bottom: 1px solid rgba(255,255,255,0.08); padding-bottom: 0.6rem;">
                    <div style="display: flex; align-items: center; gap: 0.4rem; font-weight: 700; font-size: 0.9rem; color: #fff;">
                        <span class="material-icons-round" style="color: #f39c12; font-size: 1.1rem;">rate_review</span>
                        <span>Weekly Journal Reflection</span>
                    </div>
                    <span style="font-size: 0.75rem; color: var(--text-muted);">Weekly Review</span>
                </div>
                
                <div class="input-group" style="margin-bottom: 0;">
                    <label class="crack-input-label">Week Label</label>
                    <input type="text" id="weekly-label-input" class="custom-input" value="${defaultWeekLabel}" style="font-size: 0.85rem;">
                </div>

                <div class="input-group" style="margin-bottom: 0;">
                    <label class="crack-input-label" style="color: #2ecc71; display: flex; align-items: center; gap: 0.3rem;">
                        <span class="material-icons-round" style="font-size: 1rem;">check_circle_outline</span> What Worked This Week <span style="color: #e74c3c;">*</span>
                    </label>
                    <textarea id="weekly-worked-input" class="custom-input auto-resize" placeholder="List breakthroughs, wins, habits, and strategies that paid off this week..." rows="3" style="resize: none; font-size: 0.88rem;"></textarea>
                </div>

                <div class="input-group" style="margin-bottom: 0;">
                    <label class="crack-input-label" style="color: #e74c3c; display: flex; align-items: center; gap: 0.3rem;">
                        <span class="material-icons-round" style="font-size: 1rem;">highlight_off</span> What Didn't Work This Week
                    </label>
                    <textarea id="weekly-didnt-input" class="custom-input auto-resize" placeholder="List obstacles, mistakes, distractions, or things to adjust next week..." rows="3" style="resize: none; font-size: 0.88rem;"></textarea>
                </div>

                <button type="button" class="btn-primary" style="justify-content: center; width: 100%; font-size: 0.88rem;" onclick="app.saveWeeklyReflection()">
                    <span class="material-icons-round">save</span> Save Weekly Reflection
                </button>
            </div>
        `;

        const historyCardsHTML = entries.length === 0 ? `
            <div style="text-align: center; padding: 2rem 1rem; color: var(--text-muted);">
                <span class="material-icons-round" style="font-size: 2rem; color: rgba(255,255,255,0.2); margin-bottom: 0.4rem;">folder_open</span>
                <p style="font-size: 0.85rem; margin: 0;">No weekly reflections logged yet. Fill out the form above to record your first weekly journal!</p>
            </div>
        ` : entries.map(entry => {
            const createdDate = entry.createdAt ? new Date(entry.createdAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : '';
            return `
                <div class="weekly-reflection-card">
                    <div style="display: flex; align-items: center; justify-content: space-between; border-bottom: 1px solid rgba(255,255,255,0.06); padding-bottom: 0.5rem;">
                        <div style="display: flex; align-items: center; gap: 0.4rem;">
                            <span class="material-icons-round" style="font-size: 1rem; color: #f39c12;">auto_graph</span>
                            <strong style="font-size: 0.85rem; color: #fff;">${entry.weekLabel}</strong>
                        </div>
                        <div style="display: flex; align-items: center; gap: 0.5rem;">
                            <span style="font-size: 0.72rem; color: var(--text-muted);">${createdDate}</span>
                            <button type="button" class="btn-ghost" style="padding: 0.15rem 0.35rem; color: #e74c3c;" onclick="app.deleteWeeklyReflection('${entry.id}')" title="Delete reflection">
                    ...
                    ${entry.whatWorked ? `
                    <div class="reflection-box-worked">
                        <span class="reflection-box-title" style="color: #2ecc71;">
                            <span class="material-icons-round" style="font-size: 0.9rem;">check_circle</span> WHAT WORKED
                        </span>
                        <p class="crack-text-content" style="font-size: 0.88rem; margin: 0;">${entry.whatWorked}</p>
                    </div>
                    ` : ''}

                    ${entry.whatDidntWork ? `
                    <div class="reflection-box-didnt">
                        <span class="reflection-box-title" style="color: #e74c3c;">
                            <span class="material-icons-round" style="font-size: 0.9rem;">highlight_off</span> WHAT DIDN'T WORK
                        </span>
                        <p class="crack-text-content" style="font-size: 0.88rem; margin: 0;">${entry.whatDidntWork}</p>
                    </div>
                    ` : ''}
                </div>
            `;
        ...
        container.innerHTML = `
            ...
            <div style="margin-top: 0.5rem;">
                <span style="font-size: 0.8rem; font-weight: 700; color: var(--text-muted); text-transform: uppercase; letter-spacing: 0.8px; display: block; margin-bottom: 0.75rem;">Past Weekly Journals (${entries.length})</span>
    ...
    checkAndShowGreatestCrackModal(onProceed) {
        const crack = this.getDailyCrack();

        // 1. If achieved today -> skip modal
        if (crack.achieved) {
            onProceed();
            return;
        }

        // 2. If snoozed and snooze period hasn't expired -> skip modal
        if (crack.snoozedUntil) {
            const snoozeTime = new Date(crack.snoozedUntil).getTime();
        ...
        // Save target action to proceed after modal interaction
        this.pendingProceedCallback = onProceed;

        const modalTitle = document.getElementById('crack-modal-title');
        const modalSub = document.getElementById('crack-modal-subtitle');
        const inputView = document.getElementById('crack-input-view');
        const reminderView = document.getElementById('crack-reminder-view');
        const whatInput = document.getElementById('crack-what-input');
        const howInput = document.getElementById('crack-how-input');
        const displayWhat = document.getElementById('crack-display-what');
        const displayHow = document.getElementById('crack-display-how');
        const snoozeBox = document.getElementById('crack-snooze-box');

        if (snoozeBox) snoozeBox.style.display = 'none';

        // Case A: No crack entered for today yet -> Show Input View
        if (!crack.what || !crack.what.trim()) {
            if (modalTitle) modalTitle.textContent = "What will your greatest crack be today?";
            if (modalSub) modalSub.textContent = "It can be something that you haven't tried before or something new that you have not done and can cause a biggest crack";
            if (inputView) inputView.style.display = 'flex';
            ...
            this.openModal('greatest-crack-modal');
            this.switchJournalTab('today');
            setTimeout(() => {
                if (whatInput) {
                    whatInput.focus();
                    this.resizeTextarea(whatInput);
                }
            }, 120);
        } else {
            // Case B: Crack exists, not achieved & not snoozed -> Show Reminder View
            if (modalTitle) modalTitle.textContent = "Today's Greatest Crack Focus";
            if (modalSub) modalSub.textContent = "Here is the biggest breakthrough you committed to delivering today. Keep pushing!";
            if (inputView) inputView.style.display = 'none';
            if (reminderView) reminderView.style.display = 'flex';
    ...
    saveGreatestCrackFromModal() {
        const whatInput = document.getElementById('crack-what-input');
        const howInput = document.getElementById('crack-how-input');
        const what = whatInput ? whatInput.value.trim() : '';
        const how = howInput ? howInput.value.trim() : '';

        if (!what) {
            if (whatInput) {
                whatInput.style.borderColor = '#e74c3c';
                whatInput.focus();
            }
            return;
        }
        if (whatInput) whatInput.style.borderColor = '';

        const crack = this.getDailyCrack();
        crack.what = what;
        crack.how = how;
        crack.achieved = false;
        crack.snoozedUntil = null;

        this.saveDailyCrack(crack);
        this.closeModal();

        if (this.pendingProceedCallback) {
            const cb = this.pendingProceedCallback;
            this.pendingProceedCallback = null;
            cb();
        }
    },

    proceedFromGreatestCrackModal() {
        this.closeModal();
        if (this.pendingProceedCallback) {
            const cb = this.pendingProceedCallback;
            this.pendingProceedCallback = null;
    ...
    markGreatestCrackAchieved() {
        ...
        // Toast celebration effect for achievement
        this.triggerCrackAchievementCelebration();
    ...
    toggleCrackSnoozeBox() {
        const box = document.getElementById('crack-snooze-box');
        if (!box) return;
        const isHidden = box.style.display === 'none' || !box.style.display;
        box.style.display = isHidden ? 'block' : 'none';

        if (isHidden) {
            const customInput = document.getElementById('crack-custom-snooze-time');
            if (customInput) {
                const defaultSnooze = new Date(Date.now() + 2 * 3600 * 1000);
                const localISO = new Date(defaultSnooze.getTime() - (defaultSnooze.getTimezoneOffset() * 60000)).toISOString().slice(0, 16);
                customInput.value = localISO;
            }
        }
    },

    snoozeGreatestCrackPreset(hours) {
        const snoozeUntil = new Date(Date.now() + hours * 3600 * 1000).toISOString();
        const crack = this.getDailyCrack();
        crack.snoozedUntil = snoozeUntil;
        this.saveDailyCrack(crack);
    ...
    snoozeGreatestCrackCustom() {
        const customInput = document.getElementById('crack-custom-snooze-time');
        if (!customInput || !customInput.value) return;

        const selectedDate = new Date(customInput.value);
        if (isNaN(selectedDate.getTime()) || selectedDate.getTime() <= Date.now()) {
            alert('Please select a future date and time for snoozing.');
            return;
        }

        const crack = this.getDailyCrack();
        crack.snoozedUntil = selectedDate.toISOString();
        this.saveDailyCrack(crack);
    ...
    openTaskModal(taskId = null, categoryId = null) {
        this.checkAndShowGreatestCrackModal(() => {
            this._doOpenTaskModal(taskId, categoryId);
        });
    },

    _doOpenTaskModal(taskId = null, categoryId = null) {
    ...
    calculateTotalXP(st) {
        if (!st) return { totalXP: 0, breakdown: {} };
        ...
        const eventXP = completedEvents * 150;

        const resolvedQuestions = questions.filter(q => q.resolved || (q.answers && q.answers.length > 0) || q.answer).length;
        const questionXP = resolvedQuestions * 100;

        const daysCount = days.length;
        const daysXP = daysCount * 75;

        let completedCatCount = 0;
        ...
        const categoryXP = completedCatCount * 120;

        let completionBonusXP = 0;
        if (tasks.length > 0) {
            const pct = completedTasks / tasks.length;
            completionBonusXP = Math.floor(pct * 2500);
        }

        const totalXP = taskXP + eventXP + questionXP + daysXP + categoryXP + completionBonusXP;

        return {
            totalXP,
            breakdown: {
                completedTasks, taskXP,
                completedEvents, eventXP,
                resolvedQuestions, questionXP,
                daysCount, daysXP,
                completedCatCount, categoryXP,
                completionBonusXP
            }
        };
    },
    ...
    getLevelInfo(totalXP) {
        let currentLevelObj = LEVEL_CONFIG[0];
        let nextLevelObj = LEVEL_CONFIG[1];

        for (let i = LEVEL_CONFIG.length - 1; i >= 0; i--) {
            if (totalXP >= LEVEL_CONFIG[i].minXP) {
                currentLevelObj = LEVEL_CONFIG[i];
                nextLevelObj = LEVEL_CONFIG[i + 1] || null;
                break;
            }
        }

        let progressPct = 100;
        let xpText = `${totalXP.toLocaleString()} XP (MAX LEVEL)`;
        let currentLevelMin = currentLevelObj.minXP;
        let nextLevelMin = nextLevelObj ? nextLevelObj.minXP : currentLevelMin;

        if (nextLevelObj) {
            const range = nextLevelMin - currentLevelMin;
            const gained = totalXP - currentLevelMin;
            progressPct = Math.min(100, Math.max(0, Math.floor((gained / range) * 100)));
            xpText = `${totalXP.toLocaleString()} / ${nextLevelMin.toLocaleString()} XP`;
        }
    ...
    updateLevelUI() {
        if (!state) return;
        const xpData = this.calculateTotalXP(state);
        const lvlInfo = this.getLevelInfo(xpData.totalXP);

        const badgeEl = document.getElementById('header-level-badge');
        const rankEl = document.getElementById('header-rank-title');
        const xpTextEl = document.getElementById('header-xp-text');
        const xpBarEl = document.getElementById('header-xp-bar');

        if (badgeEl) badgeEl.textContent = `Lvl ${lvlInfo.level}`;
        if (rankEl) rankEl.textContent = lvlInfo.title;
        if (xpTextEl) xpTextEl.textContent = lvlInfo.xpText;
        if (xpBarEl) xpBarEl.style.width = `${lvlInfo.progressPct}%`;

        // Trigger Level Up Toast if level increases
        if (this._lastUserLevel !== null && lvlInfo.level > this._lastUserLevel) {
            this.triggerLevelUpToast(lvlInfo);
        }
        this._lastUserLevel = lvlInfo.level;
    },

    triggerLevelUpToast(lvlInfo) {
        const toast = document.getElementById('level-up-toast');
        const detail = document.getElementById('level-up-detail');
        if (!toast || !detail) return;

        detail.textContent = `You reached Level ${lvlInfo.level}: ${lvlInfo.title}!`;
        toast.style.display = 'block';

        if (this._levelToastTimer) clearTimeout(this._levelToastTimer);
        this._levelToastTimer = setTimeout(() => {
            toast.style.display = 'none';
        }, 4500);
    },
    ...
    openLevelModal() {
        ...
        const xpSubtitle = document.getElementById('modal-xp-subtitle');
        const breakdownContainer = document.getElementById('modal-xp-breakdown');
        const roadmapContainer = document.getElementById('modal-roadmap-list');

        if (modalTitle) modalTitle.textContent = `Level ${lvlInfo.level} - ${lvlInfo.title}`;
        if (xpSubtitle) xpSubtitle.textContent = `Total Experience: ${lvlInfo.totalXP.toLocaleString()} XP`;

        if (breakdownContainer) {
            const b = xpData.breakdown;
            breakdownContainer.innerHTML = `
                <div style="background: rgba(255,255,255,0.04); padding: 0.5rem 0.65rem; border-radius: 6px;">
                    <div style="color: var(--text-muted); font-size: 0.72rem;">Completed Tasks</div>
                    <div style="font-weight: 700; color: #2ecc71;">${b.completedTasks} <span style="font-size: 0.7rem; color: var(--text-muted); font-weight: 400;">(+${b.taskXP} XP)</span></div>
                </div>
                <div style="background: rgba(255,255,255,0.04); padding: 0.5rem 0.65rem; border-radius: 6px;">
                    <div style="color: var(--text-muted); font-size: 0.72rem;">Timeline Events</div>
                    <div style="font-weight: 700; color: #3498db;">${b.completedEvents} <span style="font-size: 0.7rem; color: var(--text-muted); font-weight: 400;">(+${b.eventXP} XP)</span></div>
                </div>
                <div style="background: rgba(255,255,255,0.04); padding: 0.5rem 0.65rem; border-radius: 6px;">
                    <div style="color: var(--text-muted); font-size: 0.72rem;">Resolved Queries</div>
                    <div style="font-weight: 700; color: #f39c12;">${b.resolvedQuestions} <span style="font-size: 0.7rem; color: var(--text-muted); font-weight: 400;">(+${b.questionXP} XP)</span></div>
                </div>
                <div style="background: rgba(255,255,255,0.04); padding: 0.5rem 0.65rem; border-radius: 6px;">
                    <div style="color: var(--text-muted); font-size: 0.72rem;">Days Created</div>
                    <div style="font-weight: 700; color: #9b59b6;">${b.daysCount} <span style="font-size: 0.7rem; color: var(--text-muted); font-weight: 400;">(+${b.daysXP} XP)</span></div>
                </div>
                <div style="background: rgba(255,255,255,0.04); padding: 0.5rem 0.65rem; border-radius: 6px;">
                    <div style="color: var(--text-muted); font-size: 0.72rem;">Complete Categories</div>
                    <div style="font-weight: 700; color: #1abc9c;">${b.completedCatCount} <span style="font-size: 0.7rem; color: var(--text-muted); font-weight: 400;">(+${b.categoryXP} XP)</span></div>
                </div>
                <div style="background: rgba(255,255,255,0.04); padding: 0.5rem 0.65rem; border-radius: 6px;">
                    <div style="color: var(--text-muted); font-size: 0.72rem;">Timeline Completion Bonus</div>
                    <div style="font-weight: 700; color: #e74c3c;">+${b.completionBonusXP} XP</div>
                </div>
            `;
        }

        if (roadmapContainer) {
            roadmapContainer.innerHTML = LEVEL_CONFIG.map(cfg => {
                const isActive = cfg.level === lvlInfo.level;
                const isCompleted = cfg.level < lvlInfo.level;
                return `
                    <div class="roadmap-item ${isActive ? 'active' : ''} ${isCompleted ? 'completed' : ''}">
                        <div class="roadmap-left">
                            <div class="roadmap-badge">${cfg.level}</div>
                            <div>
                                <div class="roadmap-title">${cfg.title} ${isActive ? '<span style="font-size: 0.7rem; background: #f39c12; color: #121216; font-weight: 700; padding: 0.1rem 0.4rem; border-radius: 100px; margin-left: 0.3rem;">CURRENT</span>' : ''}</div>
                                <div class="roadmap-xp-req">${cfg.minXP.toLocaleString()} Total XP Required</div>
                            </div>
                        </div>
                        <span class="material-icons-round" style="font-size: 1.2rem; color: ${isActive ? '#f39c12' : (isCompleted ? '#2ecc71' : 'rgba(255,255,255,0.2)')};">
                            ${isCompleted ? 'check_circle' : (isActive ? 'stars' : 'lock')}
                        </span>
                    </div>
                `;
            }).join('');
